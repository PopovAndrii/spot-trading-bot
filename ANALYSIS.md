...
- ✅ **Клиентский reconnect без backoff** — `SpotWS.js:128-139`: каждые 2 с навсегда,
  ...
  > Все четыре исправлены (ветка `ws-hygiene`): heartbeat ping/pong 30 с с
  > terminate; при resubscribe сокет удаляется из старой комнаты, обработчики
  > ts используют зафиксированный `sym` вместо мутирующего closure;
  > `#maybeCleanup` убирает timerSender, когда нет клиентов и цикл не работает
  > (статус пары живёт в `pair`); клиентский reconnect — экспоненциальный
  > backoff 2с→30с, после 8 попыток стоп с предложением перезагрузить страницу.
